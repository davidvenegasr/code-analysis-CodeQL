...
class UnsafeDeserializationConfig extends TaintTracking::Configuration {
  ...
  override predicate isSink(DataFlow::Node sink) { sink.getLocation().getFile().getBaseName().matches("%PartitionExpressionForMetastore.java%") //and sink.asExpr().toString().matches("%exprBytes%") 
  																							and sink.getLocation().getStartLine() = 110}
